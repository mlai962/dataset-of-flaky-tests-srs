...
public class RepoUtil {
	...
	public static void deleteTempRepoDir() {
		try {
			FileUtils.delete(new File(dir + File.separator + lastRepoName + File.separator), org.eclipse.jgit.util.FileUtils.RECURSIVE);
	...
	public static boolean cloneRepo (Project project) {
		...
		String cloneURL = repoURL.replace("api", "www").replace("repos/", "") + ".git/";
		...
		if (isCloned) {
			int indexOfLastSlashRepoName = project.getProjectName().lastIndexOf("/");
			String repoName = project.getProjectName().substring(indexOfLastSlashRepoName+1, project.getProjectName().length());
			lastRepoName = repoName;
			
			return executeCommand("git reset " + commitHash + " --hard", dir + File.separator + repoName);
		} else {
			return false;
		}
	}
	...
	public static Boolean checkClassExists (Project project) {
		int indexOfLastSlashClassName = project.getClassName().lastIndexOf("/");
		int indexOfLastSlashRepoName = project.getProjectName().lastIndexOf("/");
		String repoName = project.getProjectName().substring(indexOfLastSlashRepoName+1, project.getProjectName().length());
		File testClassDir = new File(dir + File.separator + repoName + File.separator + project.getClassName().substring(0, indexOfLastSlashClassName+1) + File.separator);
		final String testClassName = project.getClassName().substring(indexOfLastSlashClassName+1, project.getClassName().length());
